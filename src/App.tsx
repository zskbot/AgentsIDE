...
import { Bot, Check, ChevronDown, Code2, FileCode2, Folder, GitBranch, GitPullRequest, Play, Plus, Search, Send, Settings2, Sparkles, TerminalSquare, TestTube2, X } from 'lucide-react'
...
function App() {
  ...
  return <div className="app-shell">
    <header className="topbar"><div className="brand"><span className="brand-mark"><Sparkles size={16} /></span><strong>AgentsIDE</strong></div><button className="workspace-switch">zskbot / AgentsIDE <ChevronDown size={14} /></button><div className="top-actions"><span className={`status-dot ${busy ? 'busy' : ''}`} /><span className="status-text">{busy ? 'Agent running' : task?.status === 'complete' ? 'Complete' : 'Ready'}</span><button className="icon-btn"><Settings2 size={18} /></button></div></header>
    <main className="main-content">
      {view === 'workspace' && <Workspace onOpenCode={() => openFile(activeFile)} onOpenFile={openFile} onNewTask={() => setView('agent')} />}
