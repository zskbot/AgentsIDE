...
import { AUTOSHIP_URL, ensureAgentsIDEProject, getAutoshipProjects, triggerAgentsIDEDeploy } from './features/deploy/autoship'
...
function App() {
  ...
  useEffect(() => { getAutoshipProjects().then(() => setAutoshipOnline(true)).catch(() => setAutoshipOnline(false)) }, [])
  ...
  const deploy = async () => {
    ...
    setDeployBusy(true); setDeployMessage('Connecting to Autoship…')
    try {
      const project = await ensureAgentsIDEProject()
      const run = await triggerAgentsIDEDeploy(project.id, task?.prompt ? `deploy: ${task.prompt.slice(0, 72)}` : 'deploy: AgentsIDE through Autoship')
      setAutoshipOnline(true); setAutoshipRuns(value => value + 1)
      setDeployMessage(`Pipeline ${run.id} started`)
    } catch (error) {
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      {view === 'workspace' && <Workspace onOpenCode={() => openFile(activeFile)} onOpenFile={openFile} onNewTask={() => setView('agent')} autoshipOnline={autoshipOnline} autoshipRuns={autoshipRuns} onDeploy={deploy} deployBusy={deployBusy} deployMessage={deployMessage} />}
...
function Workspace({ onOpenCode, onOpenFile, onNewTask, autoshipOnline, autoshipRuns, onDeploy, deployBusy, deployMessage }: { onOpenCode: () => void; onOpenFile: (file: string) => void; onNewTask: () => void; autoshipOnline: boolean; autoshipRuns: number; onDeploy: () => void; deployBusy: boolean; deployMessage: string }) { return <section className="screen workspace-screen"><div className="eyebrow">01 · WORKSPACE</div><div className="hero-row"><div><h1>Build with agents.</h1><p>Code, review, test and ship from one workspace.</p></div><button className="primary-btn" onClick={onNewTask}><Plus size={17} /> New task</button></div><div className="repo-card"><div className="repo-icon"><Code2 size={21} /></div><div className="repo-copy"><strong>AgentsIDE</strong><span><GitBranch size={13} /> main · clean</span></div><span className="live-pill">READY</span></div><div className="deploy-card"><div><span className="mini-label">AUTOSHIP</span><strong>Continuous deployment</strong><small>{autoshipOnline ? 'Connected to deployment API' : 'Connection not available yet'}</small></div><div className="deploy-side"><span className={`connection ${autoshipOnline ? 'online' : ''}`}><span />{autoshipOnline ? 'ONLINE' : 'OFFLINE'}</span><button className="primary-btn" onClick={onDeploy} disabled={deployBusy}><Rocket size={16} /> {deployBusy ? 'Deploying…' : 'Deploy'}</button></div>{deployMessage && <div className="deploy-message">{deployMessage} · <a href={AUTOSHIP_URL} target="_blank" rel="noreferrer">Open Autoship <ExternalLink size={12} /></a></div>}</div><div className="section-heading"><span>Repository files</span><button className="ghost-btn" onClick={onOpenCode}>Open editor <Code2 size={15} /></button></div><div className="file-list">{files.map(file => <button className={`file-row ${file.name === 'App.tsx' ? 'selected' : ''}`} key={file.path} onClick={() => onOpenFile(file.name)}><FileCode2 size={17} /><span>{file.name}</span><small>{file.path}</small></button>)}</div><div className="metrics"><Metric label="Changes" value="0" /><Metric label="Tests" value="Ready" /><Metric label="Autoship" value={autoshipRuns ? `${autoshipRuns} run` : 'Ready'} /></div></section> }
...
function Tools({ onDeploy, deployBusy, deployMessage }: { onDeploy: () => void; deployBusy: boolean; deployMessage: string }) { return <section className="screen tools-screen"><div className="eyebrow">06 · TOOLS</div><h1>Developer tools</h1><div className="tool-grid"><Tool icon={<TerminalSquare />} title="Terminal" text="Run commands in the workspace." /><Tool icon={<TestTube2 />} title="Tests" text="Run the project test suite." /><Tool icon={<GitBranch />} title="Git" text="Branches, commits and status." /><button className="tool-card" onClick={onDeploy} disabled={deployBusy}><span><Rocket /></span><strong>{deployBusy ? 'Deploying…' : 'Autoship Deploy'}</strong><small>{deployMessage || 'Ship the verified main branch.'}</small></button></div></section> }
